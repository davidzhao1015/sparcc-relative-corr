...
Use git bash commend line to run SparCC python2 module following the
package manual. The outputs from SparCC analysis were stored in the main
...
```
To simplify the heatmap plots, identified and filtered out low-abundant taxa that has less than 10 reads in more than 5 samples. 
...
Stored the two-sided p values in the data frame. And then filtered out the low abundant taxa from the full table. 
...
```{r corr matrix heatmap, fig.height=6, fig.width=8, fig.cap="Significant correlations between abundant species based on SparCC analysis"}
